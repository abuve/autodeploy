...
class Asset(object):
        ...
        if data:
            try:
                ...
                if self.mandatory_check(data, only_check_md5=True):
                    response = {'needs_aproval': 'This asset has already report to server, Please waiting the admin confirm.'}
                else:
    ...
    def save_new_asset_to_approval_zone(self):
        ...
        asset_already_in_approval_zone = CMDB_MODELS.NewAssetApprovalZone.objects.get_or_create(ipaddress=self.clean_data.get('ipaddress'),
                                                                                           sn=asset_sn,
    ...
    def __update_server_component(self):
        update_fields = ['model','os_type','os_release', 'hostname', 'CpuUsage', 'MemUsage', 'DiskUsage', 'configuration', 'Memory', 'DeviceSize', 'LoadInfo', 'ipaddress']
