...
class WebConfigNginxView(LoginRequiredMixin, PermissionRequiredMixin, View):
...
class WebConfigNginxJsonView(LoginRequiredMixin, PermissionRequiredMixin, View):
    def post(self, request, server_id):
        response = web_config.WebConfig().load_tree_data(request, server_id)
        return HttpResponse(json.dumps(response.data))


class WebConfigVersionJsonView(LoginRequiredMixin, PermissionRequiredMixin, View):
    def get(self, request, server_id):
...
class WebConfigVersionListView(LoginRequiredMixin, PermissionRequiredMixin, View):
    def get(self, request, server_id):
        response = web_config.WebConfig.get_version_tree(server_id)
        return JsonResponse(response.__dict__)


class WebConfigFileData(LoginRequiredMixin, PermissionRequiredMixin, View):
    def get(self, request, server_id):
...
class WebConfigFilePush(LoginRequiredMixin, PermissionRequiredMixin, View):
    def post(self, request, server_id):
        response = web_config.WebConfig.push_file(request, server_id)
        return JsonResponse(response.__dict__)


class WebConfigVersionStatus(LoginRequiredMixin, PermissionRequiredMixin, View):
