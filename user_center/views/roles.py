...
class RolesListView(LoginRequiredMixin, PermissionRequiredMixin, View):
    def get(self, request):
        #response = permission.PermissionConfig.nginx_config(server_id)
        return render(request, 'roles_list.html', {'response': 1})


class RolesJsonView(LoginRequiredMixin, PermissionRequiredMixin, View):
    def get(self, request):
...
class RolesAddView(LoginRequiredMixin, PermissionRequiredMixin, View):
    def get(self, request):
